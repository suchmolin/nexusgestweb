...
export default function CuentasPorCobrarPage() {
  ...
  const [clientDetail, setClientDetail] = useState<ArItem['client'] | null>(null);
  ...
  useEffect(() => {
    if (!companyId) return;
    configApi.get(companyId).then((c) => setConfig(c as { currencySymbol?: string; usdRate?: number | null; eurRate?: number | null })).catch(() => setConfig(null));
  }, [companyId]);
  ...
  useEffect(() => {
    ...
    if (convertToBs && defaultRate != null && !isNaN(defaultRate) && !rateOfDay) {
      setRateOfDay(defaultRate.toFixed(2));
    }
  }, [canConvertBs, convertToBs, defaultRate, rateOfDay]);

  const canNew = !allowedModules || hasSectionAccess('CUENTAS_POR_COBRAR', 'new', allowedModules);
  const canList = !allowedModules || hasSectionAccess('CUENTAS_POR_COBRAR', 'list', allowedModules);
  ...
  const handleCreate = async () => {
    ...
    try {
      ...
      resetForm();
      setSuccess('Cuenta por cobrar registrada.');
      if (canList) {
        setTab('list');
  ...
  const confirmMarkPaid = async () => {
    ...
    try {
      await accountsReceivableApi.markPaid(markPaidId, companyId);
      setMarkPaidId(null);
      loadList();
    } catch (e) {
  ...
  const statusBadge = (status: ArItem['status']) => {
    const map = {
        ...
        <tbody>
          {rows.map((row) => (
            <tr key={row.id} className="border-t border-[var(--border)]">
              ...
              <td className="px-3 py-2">{formatDate(String(row.createdAt))}</td>
                ...
                {!row.paid && (
                  <div className="relative inline-flex group">
                    <button
                      ...
                      className="rounded-lg p-1.5 text-[var(--success)] hover:bg-[var(--success)]/10 transition-colors"
                    >
                      <IconCheckCircle width={18} height={18} className="flex-shrink-0" />
                    </button>
                    <span
                      role="tooltip"
                      className="pointer-events-none absolute bottom-full left-1/2 -translate-x-1/2 mb-2 whitespace-nowrap rounded-md bg-[var(--foreground)] text-[var(--background)] px-2 py-1 text-xs opacity-0 invisible group-hover:opacity-100 group-hover:visible transition-opacity z-10"
                    >
                      Marcar pagada
                    </span>
                  </div>
  ...
  return (
    <div className="p-6 md:p-8">
      <h1 className="text-2xl font-bold text-[var(--foreground)]">Cuentas por cobrar</h1>
      <p className="text-[var(--muted)] mt-1">Registra y da seguimiento a cobros pendientes de clientes.</p>
        ...
        <>
          ...
          {tab === 'new' && canNew && (
            <div className="mt-6 max-w-2xl space-y-4 p-5 rounded-xl bg-[var(--card)] border border-[var(--border)]">
              ...
              >
                {saving ? 'Guardando...' : 'Registrar cuenta por cobrar'}
              </button>
            </div>
          )}
          ...
          {tab === 'list' && canList && (
            <div className="mt-6 space-y-4">
              ...
              <div className="grid grid-cols-1 md:grid-cols-4 gap-3 p-4 rounded-xl bg-[var(--card)] border border-[var(--border)]">
                <input type="date" value={filterFrom} onChange={(e) => { setFilterFrom(e.target.value); setPage(1); }} className="rounded-lg bg-[var(--background)] border border-[var(--border)] px-3 py-2" title="Desde" />
                <input type="date" value={filterTo} onChange={(e) => { setFilterTo(e.target.value); setPage(1); }} className="rounded-lg bg-[var(--background)] border border-[var(--border)] px-3 py-2" title="Hasta" />
                <input
                  value={filterName}
      ...
      {markPaidId && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/50" role="dialog" aria-modal="true">
          <div className="bg-[var(--card)] rounded-xl border border-[var(--border)] shadow-xl max-w-md w-full p-6" onClick={(e) => e.stopPropagation()}>
            ...
            <p className="text-sm text-[var(--muted)] mb-4">¿Confirmas que esta cuenta por cobrar ya fue cobrada?</p>
